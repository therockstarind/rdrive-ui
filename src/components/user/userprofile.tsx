...
const UserDetails: React.FC<{ user?: UserType }> = ({ user }) => (
  <AnimatedDiv variants={FadeContainer} className="flex flex-col gap-3">
    ...
    <Flex className="relative h-[180px] w-full rounded-t-large bg-gradient-to-br from-pink-300 via-purple-300 to-indigo-400 subpixel-antialiased">
      {/* The user's cover image displayed with optional fallback. */}
      <Image
        src={user?.coverimg || ''}
        alt=""
        className="h-[180px] overflow-hidden rounded-t-large object-cover"
        radius="none"
        height="300"
        width="1270"
      />
      {/* Edit button and user's avatar linked to their profile. */}
      <Button className="group absolute right-3 top-3 z-10 box-border h-unit-8 min-w-unit-16 whitespace-nowrap rounded-full bg-black/30 px-unit-3 text-xs text-white subpixel-antialiased backdrop-blur-md dark:bg-black/70">
        Edit
      </Button>
      <Link
        href={`/${user?.username}`}
    ...
    >
      ...
      <Text display="max-w-[90%]" size="text-sm" color="text-muted-foreground">
        @{user?.username}
      </Text>
    </Flex>
